...
public class MissionRestController {
    ...
    @Operation(summary = "내 미션 목록 조회 API",description = "내 미션들의 목록을 조회하는 API이며, 페이징을 포함합니다. query String 으로 미션 상태와 page 번호를 주세요")
    @ApiResponses({
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "COMMON200",description = "OK, 성공")
    })
    @Parameters({
            @Parameter(name = "pageNumber", description = "페이지 넘버, query string 입니다. 1부터 시작입니다."),
            @Parameter(name = "missionStatus", description = "미션 상태, query string 입니다. ONGOING/SUCCESS/FAIL 중 하나 입력")
    })
    public ApiResponse<MissionResponseDTO.myMissionListDTO> getAllByUserByPage(
            @ExistPage @RequestParam(name = "pageNumber", defaultValue = "1") Integer pageNumber,
            @RequestParam(name = "missionStatus", defaultValue = "ONGOING") MissionStatus missionStatus) {

        Page<UserMission> missionList = userMissionQueryService.getByUserByStatusByPage(1L, missionStatus, pageNumber-1, 10);

        return ApiResponse.onSuccess(MissionConverter.toMyMissionListDTO(missionList));
